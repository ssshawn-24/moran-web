  ...
  return (
    ...
    >
      ...
      <div style={{ position: "absolute", left: 98, top: 578, zIndex: 10, maxWidth: 560 }}>
        <strong style={{ display: "block", color: "var(--color-clay)", fontSize: 13, fontWeight: 840, textTransform: "uppercase", letterSpacing: "0.04em", marginBottom: 18 }}>AI · CONTENT GROWTH</strong>
        <p style={{ color: "var(--color-ink)", fontSize: 29, lineHeight: 1.45, fontWeight: 520, maxWidth: 520 }}>用 AI 驱动内容增长，从推广到增长，分享 SEO、GEO 实战方法。</p>
        </div>
      {/* Actions */}
      <div style={{ position: "absolute", left: 98, top: 720, zIndex: 10, display: "flex", alignItems: "center", gap: 36 }}>
        <button className="btn-primary" style={{ color: "var(--color-ink)", borderColor: "var(--color-ink)" }}>阅读文章</button>
      ...
      <div style={{ position: "absolute", right: 86, bottom: 82, width: 314, textAlign: "right", zIndex: 10 }}>
        <p style={{ color: "var(--color-ink)", opacity: 0.56, fontSize: 13, lineHeight: 1.7 }}>这里不是一张名片，而是一个正在迭代的内容增长实验室。</p>
      </div>
